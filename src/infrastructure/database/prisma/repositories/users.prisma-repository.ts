    ...
    IUpdateUsersRepository
{
  ...
  public async findById(parameters: FindByIdUsersRepositoryDTO.Parameters): FindByIdUsersRepositoryDTO.Result {
    try {
      ...
      return success({ user: { id: new Id({ id: foundUser.id }) } });
    } catch (error: any) {
      const repositoryError = new RepositoryError({
        error,
        repository: {
  ...
  public async findByUsername(
    parameters: FindByUsernameUsersRepositoryDTO.Parameters
  ): FindByUsernameUsersRepositoryDTO.Result {
    try {
      const foundUser = await this.prisma.usersTable.findFirst({
        where: { username: parameters.username.value }
      });
    ...
    try {
      ...
      const created = await this.prisma.usersTable.create({
        data: {
          ...
          email: parameters.email.value,
          isEmailValidated: parameters.isEmailValidated,
          facebookAccountId: parameters.facebookAccountId,
          password: id.value,
          username: parameters.username.value
        }
      });
    ...
    try {
      ...
      const created = await this.prisma.usersTable.create({
        data: {
          ...
          email: parameters.email.value,
          isEmailValidated: parameters.isEmailValidated,
          googleAccountId: parameters.googleAccountId,
          password: id.value,
          username: parameters.username.value
        }
      });
